...
mysql_db = mysql.connector.connect(
    host="localhost", user="root", password="root", database="codigo"
)

# Conexion para postgres
postgres_db = psycopg2.connect(
    host="localhost", user="postgres", password="root", database="codigo", port="5432"
)
    ...
    if method == "GET":
        try:
            ...
            for alumno in record:
                response.append(
                    {"id": alumno[0], "nombre": alumno[1], "apellido": alumno[2]}
                )
            return response, 200
        ...
        try:
            ...
            mysql_db.commit()
            return {"message": "Alumno registrado exitosamente"}, 200
        except Exception as e:
