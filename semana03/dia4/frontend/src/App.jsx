...
class App extends React.Component {
  ...
  componentDidMount() {
    ...
    axios.get("http://localhost:5000/tarea").then((res) => {
      console.log(res.data.content);
      this.setState({ tareas: res.data.content });
    });
  }
  ...
  guardar(e) {
    ...
    if (cod > 0) {
      //actualizar
      axios.put("http://localhost:5000/tarea/" + cod, dataTarea).then((res) => {
      ...
      //agregar
      axios.post("http://localhost:5000/tarea", dataTarea).then((res) => {
        console.log(res.data.content);
  ...
  mostrar(cod, index) {
    axios.get("http://localhost:5000/tarea/" + cod).then((res) => {
      this.setState({
        pos: index,
  ...
  eliminar(cod) {
    axios.delete("http://localhost:5000/tarea/" + cod).then((res) => {
      console.log(res.data.content);
      var temp = this.state.tareas.filter(
        (tarea) => tarea.id !== res.data.content.id
      );
      this.setState({
        tareas: temp,
      });
